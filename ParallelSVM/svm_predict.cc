...
string predictFLAGS_model_path = ".";
string predictFLAGS_output_path = ".";
int predictFLAGS_batch_size = 10000;
//=============================================================================
...
void ParseCommandLineForPredict(int* argc, char*** argv) {
	int i;
	for (i = 1; i < *argc; ++i) {
		if ((*argv)[i][0] != '-') break;
		if (++i >= *argc) {
		...
		if (strcmp(param_name, "batch_size") == 0) {
			predictFLAGS_batch_size = atoi(param_value);
		}
		else if (strcmp(param_name, "model_path") == 0) {
			predictFLAGS_model_path = string(param_value);
		}
		else if (strcmp(param_name, "output_path") == 0) {
			predictFLAGS_output_path = string(param_value);
		}
		else {
...
int mainForPredict(int argc, char** argv) {
	...
	ParseCommandLineForPredict(&argc, &argv);
	if (predictFLAGS_batch_size <= 0 || argc != 2) {
		UsagePredict();
		return 3;
	}
	...
	predictor.ReadModel(predictFLAGS_model_path.c_str());
	...
	predictor.PredictDocument(data_file.c_str(),
		(predictFLAGS_output_path + "/PredictResult").c_str(),
		predictFLAGS_batch_size,
		&result);
